...
{
    const std::set<uint32> blockedMaps = {
        249, // Onyxia's Lair
    ...
    // Check if the map is blocked
    if (blockedMaps.find(mapid) != blockedMaps.end()) {
        LOG_ERROR("entities.player", "Player ({}, name: {}) tried to enter a blocked map {}", GetGUID().ToString(), GetName(), mapid);
        GetSession()->SendNotification("You are not allowed to enter this raid or dungeon.");
        return false; // Prevent teleportation
    }
